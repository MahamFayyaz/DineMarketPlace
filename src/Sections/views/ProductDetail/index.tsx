...
import StripeCheckOutButton from '@/Sections/Checkout';
...
export const ProductDetail: FC<{ item: oneProductType }> = ({ item }) => {
  const [ImageforPreviewofSelector, setImageforPreviewofSelector] = useState<string>(
    item.Image[0]._key
  );
      ...
      <div>
        <div className='flex flex-col md:flex-row lg:flex-row items-center justify-between'>
          <div className='flex justify-evenly items-center'>
            {item.Image.map((subItem: imagesType, index: number) => (
              <div
                key={subItem._key}
                className='w-[28rem]'
                onClick={() => setImageforPreviewofSelector(subItem._key)}
              >
                <Image
                  width={2000}
                  height={2000}
                  src={urlFor(subItem).height(2000).width(2000).url()}
                  alt={subItem.alt}
                />
              </div>
            ))}
          ...
          <div className='space-y-2'>
            ...
            <div className='mt-3'>
              ...
              <div className='flex gap-4'>
                {item.size.map((subItem: string, index: number) => (
                  <div
                    key={subItem}
                    className='cursor-pointer rounded-full bg-gray-100 w-12 h-12 flex flex-col md:flex-row lg:flex-row justify-center items-center'
                  >
                    {subItem}
                  </div>
                ))}
            ...
            <div className='flex items-center gap-x-5 text-2xl font-bold'>
              <AddtoCart />
              <p>${item.Price}</p>
            </div>
            <div>
...
//                 {
//                 item.Image.map((subItem:imagesType,index:number)=>(
//                     <div key={subItem._key} className='w-[28rem]' onClick={()=>setImageforPreviewofSelector(subItem._key)}>
